...
import { SuspectProfileDto, UpdateSuspectProfileDto,SendOtpDto, } from 'src/dto/users.dto';
import { FingerPrintDto } from 'src/dto/fingerprint.dto';
...
export class SuspectService {
    ...
    async createSuspectProfile(suspectprofile:SuspectProfileEntity){
    ...
    async findById(id: string) {
        return this.suspectsRepository.findOneBy({
          id,
        });
    }

    async updateSuspect(id: string, suspectProfile: SuspectProfileEntity) {
        const suspect = await this.findById(id);
        if (!suspect) throw new Error('suspect does not exist');
        await this.suspectsRepository.update(id, suspectProfile);
    }

    async deleteSuspect(id: string) {
        const suspect = await this.findById(id);
        if (!suspect) throw new Error('suspect does not exist');
        await this.suspectsRepository.delete(id);
    }

    async getAllSuspects() {
        const result = await this.suspectsRepository.find();
        return result;
      }
    ...
    // async createKnownAssociates(id: string,knownAssociates:Partial<KnownAssociateEntity>){
    //     const suspectprofile = await this.suspectsRepository.findOneBy({id});
    //     if (!suspectprofile)
    //         throw new HttpException(
    //             'User not found.Cannot create Post',
